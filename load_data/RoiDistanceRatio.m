...
[tmp1, roiIndices]=intersectCols(view.coords, view.ROIs(sourceRoi).coords);
...
elseif isfield(view.rm.retinotopyModels{1}, 'exponent')
    exp=view.rm.retinotopyModels{1}.exponent(roiIndices);
end
...
if isfield(view.rm.retinotopyModels{1}, 'exp') || isfield(view.rm.retinotopyModels{1}, 'exponent')
    data.exp=exp;
end
